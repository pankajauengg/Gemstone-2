...
#GPIO.setup(VRL_Pin,GPIO.OUT)
#pi_pwm = GPIO.PWM(VRL_Pin,1000)		#create PWM instance with frequency
#pi_pwm.start(0)	

# GPIO.setmode(GPIO.BOARD)
# GPIO.setup(Motor_Pin, GPIO.OUT)
# GPIO.setup(Pump_Pin, GPIO.OUT)
# GPIO.setup(ZFL_Pin, GPIO.OUT)
    ...
    def __init__(self, **kwargs):
        ...
        def my_callback(dt):
           ...
           # count =1
            ...
            self.ids.time.text='05:45PM'
            self.ids.date.text='             '+'30th January 2019'
            
            #pi_pwm.ChangeDutyCycle(duty)
            if RPM<0 :
                print ('RPM<0')
                #GPIO.output(ZFL_Pin, GPIO.HIGH)
            else: 
                print ('RPM>0')
                #GPIO.output(ZFL_Pin, GPIO.LOW)
